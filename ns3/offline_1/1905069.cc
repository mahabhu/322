...
int main(int argc, char* argv[])
{
    ...
    cmd.AddValue ("speed", "Speed of each nodes in mobile network", speed);
    ...
    if(argc>=5) mobile = atoi(argv[4]);
    if(argc>=6) (mobile)? speed = atof(argv[5]) : coverageFactor = atof(argv[5]);
    if(argc>=7) x = atoi(argv[6]);
    if(argc>=8) y = atoi(argv[7]);
